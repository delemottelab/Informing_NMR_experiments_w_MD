...
def get_significant(my_path, ppm_cutoff=0.2):
    ...
    assert os.path.isfile(my_path), 'nc file does not exist.'
    ...
    mask = np.logical_and(np.abs(diff_means) > ppm_cutoff, np.abs(effect_size) > 0.5)
